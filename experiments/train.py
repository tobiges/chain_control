...
from cc.env.wrappers import AddRefSignalRewardFnWrapper, VideoWrapper
...
model_e1 = eqx.tree_deserialise_leaves(
    f"models/multi_model_e1.eqx", model_e1)
...
model_e2 = eqx.tree_deserialise_leaves(
    f"models/multi_model_e2.eqx", model_e2)
...
model_e3 = eqx.tree_deserialise_leaves(
    f"models/multi_model_e3.eqx", model_e3)
...
if env_id == "all":
    ...
    model = { 
        f"model_e{env_id}" : model_x[f"model_e{env_id}"],
    }
    tracker = [Tracker(f"model_e{env_id}", "train_mse")]
...
eqx.tree_serialise_leaves(f"controllers/mm_state_{state_dim}_env{env_id}.eqx", controller_trainer.trackers[0].best_model_or_controller())