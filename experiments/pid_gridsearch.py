...
def train_and_judge(config) -> float:
    ...
    source = collect_random_step_source(env, seeds=list(range(30)), amplitude=5.0)
...
search_space = {
    "p": tune.grid_search(np.linspace(0.9, 1.1, 11)),
    "i": tune.grid_search(np.linspace(0.4, 0.5, 11)),
    "d": tune.grid_search(np.linspace(-0.6, -0.7, 11)),
}
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(
        ...
        time_budget_s=24 * 3600 * 10,  # maximum runtime in seconds
